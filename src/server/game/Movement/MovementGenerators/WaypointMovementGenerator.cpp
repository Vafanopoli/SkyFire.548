...
void WaypointMovementGenerator<Creature>::LoadPath(Creature* creature)
{
    ...
    if (!i_path)
    {
        ...
        SF_LOG_ERROR("sql.sql", "WaypointMovementGenerator::LoadPath: creature %s (Entry: %u GUID: %u DB GUID: %u) doesn't have waypoint path id: %u", creature->GetName().c_str(), creature->GetEntry(), creature->GetGUIDLow(), creature->GetDBTableGUIDLow(), path_id);
        return;
    }
...
void FlightPathMovementGenerator::DoEventIfAny(Player* player, TaxiPathNodeEntry const& node, bool departure)
{
    ...
    {
        SF_LOG_DEBUG("maps.script", "Taxi %s event %u of node %u of path %u for player %s", departure ? "departure" : "arrival", eventid, node.index, node.path, player->GetName().c_str());
...
void FlightPathMovementGenerator::InitEndGridInfo()
{
    ...
    std::size_t nodeCount = (*i_path).size();        //! Number of nodes in path.
...
void FlightPathMovementGenerator::PreloadEndGrid()
{
    ...
    if (endMap)
    {
        SF_LOG_INFO("misc", "Preloading rid (%f, %f) for map %u at node index %u/%u", _endGridX, _endGridY, _endMapId, _preloadTargetNode, (uint32)(i_path->size()-1));
        endMap->LoadGrid(_endGridX, _endGridY);
    }
    else
        SF_LOG_INFO("misc", "Unable to determine map to preload flightmaster grid");
}
